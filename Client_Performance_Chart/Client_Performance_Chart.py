...
while True:
    ...
    while True:
        try:
            ...
            if len(group) > 1:
                ...
                try:
                    var = pd.concat(
                        groupstring, axis=0, ignore_index=True, keys=var1.columns.tolist()
                    )
                except ValueError:
    ...
    for i in range(lower, upper + 1):
        try:
            summ = sum(
                var[
                    pd.DataFrame(var.columns)[0][
                        pd.DataFrame([x[-12:] for x in var.columns])[0] == f"{i}NetDepWD"
                    ]
                ]
                .reset_index()
                .iloc[:, -1]
    ...
    chart["Account Percent Change"][0] = (
        str(
            round(
                100
                * (value[-1] - chart["Adjusted Opening Balance"][0])
                / math.fabs(chart["Adjusted Opening Balance"][0]),
                2,
            )
        )
        + "%"

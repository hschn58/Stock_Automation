...
earn = earnings[(earnings['ticker']==ticker) & (earnings['startdatetime'].dt.year > 1999)].dropna()
...
if len(div1) == 0:
    fig, ax = plt.subplots()
    plt.bar(earn1['Year'], earn1['Earnings'], color = ['blue'], label ='Earnings per share', width = 0.25)
    ...
    plt.bar(earn1['Year']+0.000, earn1['Earnings'], color = ['blue'], label ='Earnings per share', align='edge', width = 0.25)
    plt.bar(div1['Year']+0.25, div1['Dividends'], color =['orange'],  label ='Dividend per share', align='edge', width = 0.25)
    plt.legend(loc = 'upper left')
    plt.ylabel('Dollars per share')
    ax.axhline(color='black', linewidth=0.51)
    plt.title(ticker)

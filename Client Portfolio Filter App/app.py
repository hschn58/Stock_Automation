...
def _assign_colors(labels, *, kind):                                                      # 27
    ...
    used = set(reg.values()) | {pref.get(k) for k in map(_norm_label, pref.keys()) if pref.get(k)}  # 33
    colors = []                                                                           # 34
...
def prime_color_maps(all_classes, all_segments):                                          # 59
    ...
    if all_classes: _assign_colors(sorted(all_classes, key=lambda x: str(x).lower()), kind='class')   # 61
    if all_segments: _assign_colors(sorted(all_segments, key=lambda x: str(x).lower()), kind='segment')# 62
# --------------------------------------------------------------------------------------  # 63
...
def filter_by_sector(df, sector, operator_, target_str):
    ...
    out = (
        df.merge(keep_pairs.assign(__keep=1),
                 on=[COL_PORTFOLIO, COL_SHORTNAME],
                 how='inner')
          .drop(columns='__keep')
    )
    return out
...
def generate_pie_chart(portfolio, shortName=None, threshold=3):                              # 01
    ...
    if df_filtered.empty:                                                                    # 06
        return None                                                                          # 07
    class_values = (df_filtered.groupby(COL_CLASS)[COL_VALUE]                                # 08
                               .sum().sort_values(ascending=False))                          # 09
    ...
    fig.legend(wedges, legend_labels, title="Class", loc='lower center',                     # 30
               bbox_to_anchor=(0.5, 0.02), ncol=2, frameon=True)                             # 31
    buf = io.BytesIO()                                                                       # 32
    fig.savefig(buf, format='png', dpi=144)                                                  # 33
    buf.seek(0); plt.close(fig)                                                              # 34
...
def generate_sector_pie_chart(portfolio, short_name=None, threshold=2.5):                    # 01
    ...
    if df_filtered.empty:                                                                     # 06
        return None                                                                           # 07
    sector_values = (df_filtered.groupby(COL_SEGMENT)[COL_VALUE]                              # 08
                               .sum().sort_values(ascending=False))                           # 09
    ...
    fig.subplots_adjust(left=0.06, right=0.94, top=0.86, bottom=0.25)                         # 29
    fig.legend(wedges, legend_labels, title="Segment", loc='lower center',                    # 30
               bbox_to_anchor=(0.5, 0.02), ncol=2, frameon=True)                              # 31
    buf = io.BytesIO()                                                                         # 32
    fig.savefig(buf, format='png', dpi=144)                                                   # 33
    buf.seek(0); plt.close(fig)                                                               # 34
...
def load_data_if_needed():                                                              # 01
    ...
    acct1_col = find_col(df1.columns, 'Account Number', 'Account #', 'Acct #',          # 13
                         'Acct Number', 'AccountNo', 'Account', required=True)          # 14
    acct2_col = find_col(df2.columns, 'Account #', 'Account Number', 'Acct #',          # 15
                         'Acct Number', 'AccountNo', 'Account', required=True)          # 16
    short2_col = find_col(df2.columns, 'Short Name', 'ShortName', 'Short_Name',         # 17
                          required=True)                                                # 18
                                                                                         # 19
    ...
    map_df = (df2.dropna(subset=['__acct_key'])                                         # 31
                 [[ '__acct_key', short2_col ]].drop_duplicates(subset=['__acct_key'],  # 32
                                                                keep='first'))          # 33
    map_df = map_df.rename(columns={short2_col: 'Short Name_MAP'})                      # 34
    map_df['Short Name_MAP'] = (map_df['Short Name_MAP']                                 # 35
                                .astype(str).str.strip().replace({'': pd.NA}))          # 36
                                                                                         # 37
...
# else proceed
    ...
    COL_SHORTNAME = find_col(col_list, 'Short Name', 'ShortName', 'Short_Name',           # 03
                             required=True)                                               # 04
    COL_CLASS     = find_col(col_list, 'Class', 'Asset Class', required=True)             # 05
    COL_VALUE     = find_col(col_list, 'Market Value', 'MarketValue',                     # 06
                             'Value', required=True)                                      # 07
    COL_ACCOUNT   = find_col(col_list, 'Account Number', 'Account #', 'Acct #',           # 08
                             'Acct Number', 'AccountNo', 'Account', required=True)        # 09
...
def download():
    ...
    if selected_sector and selected_sector != '(none)':
        ...
        if not df_sec.empty:
            df_sec_sum = (
                ...
                .sum().rename('sector_value').reset_index()
            )
            sector = True
    ...
    df_port_sum = (
        ...
        .sum().rename('total_value').reset_index()
    )
    ...
    if classFilter1:
        df_class1_sum = (
            ...
            .sum().rename('class1_value').reset_index()
        )
    ...
    if use_second_filter and classFilter2:
        df_class2_sum = (
            ...
            .groupby([COL_PORTFOLIO, COL_SHORTNAME])[COL_VALUE]
            .sum().rename('class2_value').reset_index()
        )
        df_merge = df_merge.merge(df_class2_sum, on=[COL_PORTFOLIO, COL_SHORTNAME], how='left')
    ...
    for _, row in df_merge.iterrows():
        ...
        for _, r in cd.iterrows():
            if r[COL_VALUE] > 0:
                c_rows.append({
                    'accountNumber': r[COL_ACCOUNT],
                    'marketValue': r[COL_VALUE]
                })

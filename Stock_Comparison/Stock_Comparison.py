...
class StockApp:
    def __init__(self, root):
        ...
        self.time_unit = ttk.Combobox(
            root, values=["days", "months", "years"], state="readonly", width=8
        )
        ...
        self.baseline_stock = ttk.Combobox(
            root, textvariable=self.baseline_stock_var, state="disabled", width=10
        )
        ...
        tk.Button(root, text="Add", command=self.add_ticker, width=8).grid(
            row=1, column=2, sticky="w", padx=(6, 0)
        )
        tk.Button(root, text="Remove Selected", command=self.remove_selected, width=16).grid(
            row=1, column=4, columnspan=2, sticky="w", padx=(10, 0)
        )

        # Stock list
        self.stock_list = tk.Listbox(
            root, height=10, width=22, selectmode=tk.EXTENDED, exportselection=False
        )
    ...
    def add_ticker(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Invalid Ticker", f"{ticker} does not exist or data unavailable.\nError: {str(e)}"
            )
    ...
    def generate_heatmap(self):
        ...
        for ticker in self.stocks:
            try:
                ...
                messagebox.showerror(
                    "Data Error", f"Failed to fetch data for {ticker}.\nError: {str(e)}"
                )
                return
        ...
        pct_changes = pd.DataFrame(
            {t: data_dict[t].reindex(all_times).fillna(0) for t in self.stocks}
        )
        ...
        baseline_smoothed = pd.Series(
            gaussian_filter1d(baseline_pct.to_numpy(), sigma=1), index=baseline_pct.index
        )

        deviations = {}
        for t in self.stocks:
            stock_smoothed = pd.Series(
                gaussian_filter1d(pct_changes[t].to_numpy(), sigma=1), index=pct_changes.index
            )
        ...
        for t, s in deviations.items():
            deviations_ma[t] = s.rolling(
                window=win, center=True, min_periods=max(1, win // 3)
            ).mean()
        ...
        for i, t in enumerate(self.stocks):
            ...
            signed_mask = np.where(
                dev_trend >= threshold, 1.0, np.where(dev_trend <= -threshold, -1.0, 0.0)
            )
        ...
        if n:
            ...
            axs[-1].set_xticklabels(
                [time_labels[j] for j in positions], rotation=45, ha="right", fontsize=8
            )
